...
impl TicketStoreClient {
    pub fn insert(&self, draft: TicketDraft) -> Result<TicketId, TrySendError<Command>> {
        ...
        if let Err(res) = self.sender.try_send(Command::Insert { draft, response_channel: sender }){
            Err(res)
        }else{
    ...
    pub fn get(&self, id: TicketId) -> Result<Option<Ticket>, TrySendError<Command>> {
        let (sender, receiver) = std::sync::mpsc::sync_channel(10);
        if let Err(res) = self.sender.try_send(Command::Get { id, response_channel: sender }){
            Err(res)
        }else{
            Ok(receiver.recv().unwrap())
        }
